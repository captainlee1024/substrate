...
mod execution {
		...
		B: Backend<H>,
	{
		...
		overlay: &'a mut OverlayedChanges, // 保存执行后的结果, 因为stateMachine要先保存结果等块最终确定再提交
		extensions: Extensions, // 注册给该stateMachine使用的拓展
		storage_transaction_cache: Option<&'a mut StorageTransactionCache<B::Transaction, H>>, // tx cache
