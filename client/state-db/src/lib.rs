...
//! 规范化。
//! 规范化窗口跟踪由标头哈希标识的块树。内存覆盖允许获取插入窗口内任何块中的任何trie节点。
//! 覆盖网络将记录到后备数据库，并在启动时重建。在规范化窗口中，限制为 32 个块，这些块可能具有相同的块号。
//! 规范化函数从树的顶部选择一个根，并丢弃所有其他根及其子树。
//! 规范化后，所有插入块中的trie节点都将添加到后备数据库中，并且块跟踪将移动到修剪窗口，其中不允许分叉。
//! 规范化与终结性
//! 数据库引擎使用规范性的概念，而不是最终性的概念。从共识引擎的角度来看，规范区块可能尚未最终确定，但它仍然无法在数据库中恢复。
//! 大多数情况下，在正常操作期间，最后一个规范块与上次完成的块相同。
//! 但是，如果最终性由于某种原因长时间停滞不前，则只有一定数量的块可以容纳在非规范覆盖中，因此可能会强制对未最终确定的块进行规范化。
//! 修剪。
//! 有关修剪算法的详细信息，请参阅RefWindow。 在每个规范化上进行修剪，直到满足修剪约束。 StateDb
